...
class Settings:
    ...
    ai_contributor_matching_timeout_seconds: float = field(
        default_factory=lambda: float(
            os.environ.get("AI_CONTRIBUTOR_MATCHING_TIMEOUT_SECONDS", "75")
        )
    )
    ...
    # Semantic Matching (not registered as a route yet — see AI #11). Carried so
    # the package imports cleanly on `main` while its I/O contract is reworked.
