...
class GuidanceApprovedSkillSnapshot(ContractModel):
    ...
    @field_validator("evidence_id", "name", "evidence_summary")
    @classmethod
...
class GuidanceEvidenceCapsule(ContractModel):
    ...
    @field_validator("evidence_id", "label", "summary")
    @classmethod
    def evidence_text_not_blank(cls, value: str | None) -> str | None:
        if value is None:
            return None
        value = value.strip()
        if not value:
...
class SkillGapGuidanceInput(ContractModel):
    ...
    requirements: list[GuidanceRequirementSnapshot] = Field(min_length=1)
    approved_skills: list[GuidanceApprovedSkillSnapshot] = Field(default_factory=list)
    evidence: list[GuidanceEvidenceCapsule] = Field(default_factory=list)
    allowed_evidence_ids: list[str] = Field(default_factory=list)
    requested_at: datetime
    contract_version: Literal["skill-gap-guidance-v1"]
...
class GuidanceProviderOutput(ContractModel):
    missing_skills: list[GuidanceMissingSkill] = Field(default_factory=list, max_length=20)
    ...
    sources: list[GuidanceSource] = Field(min_length=1, max_length=50)
...
class SkillGapGuidanceResult(ContractModel):
    ...
    learning_resources: list[GuidanceLearningResource] = Field(default_factory=list)
    practice_projects: list[GuidancePracticeProject] = Field(default_factory=list)
    improvement_path: list[GuidanceImprovementStep] = Field(default_factory=list)
