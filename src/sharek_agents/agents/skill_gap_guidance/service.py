...
async def _invoke_with_retries(
    fn: Callable[[], Awaitable[SkillGapGuidanceProviderResponse]],
    max_retries: int = 1,
...
def _validate_input_scope(input_data: SkillGapGuidanceInput) -> set[str]:
    ...
    supplied.update(f"requirement:{req.id}" for req in input_data.requirements)
    if supplied != allowed:
        raise SkillGapGuidanceProviderError(
...
def _validate_provider_scope(
    ...
    if any(res.url not in allowed_resource_urls for res in output.learning_resources):
        raise SkillGapGuidanceProviderError(
...
async def generate_skill_gap_guidance(
    ...
    curated_resources = retrieve_curated_resources(input_data)
    allowed_resource_urls = {res.url for res in curated_resources}

    started = time.monotonic()
    try:
        if provider is not None:
            raw_response = await provider(input_data)
            response = _coerce_provider_response(raw_response)
        else:
            response = await _invoke_with_retries(lambda: _invoke_provider(input_data))
    except SkillGapGuidanceProviderSystemLimit:
    ...
    _validate_provider_scope(
        ...
        allowed_resource_urls=allowed_resource_urls,
    )
    ...
    return SkillGapGuidanceResult(
        status="COMPLETED",
        missing_skills=response.output.missing_skills,
        recommended_technologies=response.output.recommended_technologies,
        learning_resources=response.output.learning_resources,
        practice_projects=response.output.practice_projects,
        improvement_path=response.output.improvement_path,
